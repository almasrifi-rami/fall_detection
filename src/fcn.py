...
from utils.models import fcn_model
...
for _, ds in enumerate(load_data()):
    ...
    model = fcn_model(input_shape=X_train.shape[1: ], nb_classes=n_classes)
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            f'fcn/{idx}.h5', save_best_only=True, monitor='val_loss'
        ),
    ...
    model = keras.models.load_model(f'fcn/{idx}.h5')
...
with open('fcn/results.pkl', 'wb') as fh:
