...
from utils.models import inception_model
...
for _, ds in enumerate(load_data()):
    ...
    model = inception_model(input_shape=X_train.shape[1: ], nb_classes=n_classes)
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            f'inception/{idx}.h5', save_best_only=True, monitor='val_loss'
        ),
    ...
    model = keras.models.load_model(f'inception/{idx}.h5')
...
with open('inception/results.pkl', 'wb') as fh:
