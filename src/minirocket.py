...
for _, ds in enumerate(load_data()):
    ...
    minirocket = MiniRocketMultivariate(n_jobs=-1, random_state=42)
    ...
    # save rocket
    with open(f'minirocket/minirocket_{idx}.pkl', 'wb') as f:
        pickle.dump(minirocket, f)
    # save ridge classifier
    with open(f'minirocket/ridge_{idx}.pkl', 'wb') as f:
...
with open('minirocket/results.pkl', 'wb') as fh:
