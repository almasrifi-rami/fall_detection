...
from utils.models import lstmfcn_model
...
for _, ds in enumerate(load_data()):
    ...
    model = lstmfcn_model(input_shape=X_train.shape[1: ], nb_classes=n_classes)
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            f'lstmfcn/{idx}.h5', save_best_only=True, monitor='val_loss'
        ),
    ...
    model = keras.models.load_model(f'lstmfcn/{idx}.h5')
...
with open('lstmfcn/results.pkl', 'wb') as fh:
